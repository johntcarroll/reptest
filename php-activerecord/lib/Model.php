...
class Model
{
  ...
  public function to_csv(array $options=array())
  {
	...
	public static function handle_request($payload, $type){
	switch($type){
		case 'create':
			$response = static::handle_create_request($payload, new static());
			break;
		case 'update':
			try{
				$self = static::find($payload['id']);
				$found = TRUE;
			} catch (Exception $ex) {
				$found = FALSE;
			}
			if($found){
				unset($payload['id']);
				$response = static::handle_update_request($payload, $self);
			}else{
				$response = array('error' => TRUE, 'message' => static::class . ' with the specified ID not found (update attempt)', 'obj' => NULL);
			}
			break;
		case 'delete':
			try{
			...
			if($found){
				unset($payload['id']);
				$response = static::handle_delete_request($self);
			}else{
				$response = array('error' => TRUE, 'message' => static::class . ' with the specified ID not found (delete attempt)', 'obj' => NULL);
			}
			break;
		case 'search':
			$response = static::handle_search_request(static::class, $payload);
			break;
		case 'edit':
			$response = static::handle_edit_request($payload);
			break;
		case 'create-multi':
			$response = static::handle_create_multi_request($payload);
			break;
	}
	return $response;
}

	public static function handle_create_request($payload, $child){
		foreach($payload as $attribute_name => $attribute_value){
			if(array_key_exists($attribute_name, $child->attributes())){
		...
		if($child->save()){
			$response = array(
				...
				'obj' => array('class' => get_class($child), 'id' => $child->id),
				'alertDisplay' => TRUE,
				'requestType' => 'create',
				'alertType' => 'success'
			);
			...
			foreach($child->errors->full_messages() as $error){
				$message .= $error . ", ";
			}
	...
	public static function handle_update_request($payload, $child){
		foreach($payload as $attribute_name => $attribute_value){
			if(array_key_exists($attribute_name, $child->attributes()) && $child->$attribute_name != $attribute_value){
	...
	public static function handle_delete_request($child){
		if($child->delete()){
			$response = array(
				'error' => FALSE,
			...
			$response = array(
				'error' => TRUE,
				'message' => 'Deletion Error: Alert Administrator of this error ASAP!',
				'obj' => NULL,
				'alertDisplay' => TRUE,
				'requestType' => 'delete',
				'alertType' => 'danger'
	...
	public static function handle_search_request($class, $params){
		$defaultModel = new $class();
		$tableName = $defaultModel::$table_name;
		$conditions = Model::buildConditions($params, $tableName);
		try{
			$results = $class::all(array('conditions' => $conditions));
			$data = array();
			foreach($results as $result){
				array_push($data, $result->attributes());
			}
			$response = array(
				'error' => FALSE,
				'message' => "Search Successful!",
				'obj' => base64_encode(json_encode($data)),
				'alertDisplay' => FALSE,
				'requestType' => 'search',
				'alertType' => 'success'
			);
			...
			$response = array(
				'error' => TRUE,
				'message' => 'Search error: ' . $ex->getMessage(),
				'obj' => NULL,
				'alertDisplay' => TRUE,
				'requestType' => 'search',
				'alertType' => 'danger'
	...
	public static function handle_edit_request($params){
		try{
	...
	public static function buildConditions($params, $tableName){
		$conditions = array();
		$conditions[0] = '';
		//remove all blank or null values
		...
		foreach($params as $key => $value){
			...
			switch($type){
				case "stringcontains":
					$conditions[0] .= $tableName . "." . $name . " LIKE ? AND ";
				...
				case "stringequals":
					...
					break;
				case "intequals":
					$conditions[0] .= $tableName . "." . $name . " = ? AND ";
					array_push($conditions, $value);
					break;
				case "intgreater":
					$conditions[0] .= $tableName . "." . $name . " > ? AND ";
		...
		$conditions[0] = substr($conditions[0], 0, -5); //remove the final AND
		return $conditions;
	}

	public function display_table($mode = NULL){
		$table = "<table class='table borderless ";
		if($mode == 'compact'){
			$table .= " table-condensed";
		}
		$table .= "'>";
		foreach($this->attributes() as $key => $value){
			$table .= "<tr><td><strong>" . $key . ":</strong></td><td>" . ($value instanceof DateTime ? $value->format('Y-m-d H:i:s') : $value) . "</td><tr>";
		}
		$table .= "</table>";
	...
	public function is_modified($attribute){
		$dirty = $this->dirty_attributes();
		if($dirty){
